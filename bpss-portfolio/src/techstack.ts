const categories = {
    Frontend: ["HTML", "CSS", "JavaScript", "React", "ZOD", "Next.js", "TypeScript","Redux","React Native", "EXPO"],
    Backend: ["Node.js", "Express", "MongoDB", 'Prishma', "Postgress SQL", "Mongoose", "NGINX"],
    "UI Libraries": ["Tailwind CSS", "Chakra UI", "ShadCN", "Accenternity UI"],
    "DevOps & Cloud": ["AWS Services", "Docker", "Git", "GitHub", "Vercel Deployment's","GitHub Actions","Prometheus","Grafana"],
    "Storage & BaaS": ["AWS S3", "AppWrite BaaS", "Database Management", "Cloud Storage"],
    "Tools" : [
    "Adobe PRE-PRO",
    "Cap-Cut",
    "CANVA",
    "FIGMA",
    "NOTION",
    "Content Creation",
    "Social Media MNG"]
};

export default categories